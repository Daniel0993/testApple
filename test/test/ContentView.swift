...
class CartManager: ObservableObject {
    @Published var cartItems: [(String, Double, Int)] = []
    
    var totalPrice: Double {
        cartItems.reduce(0) { $0 + ($1.1 * Double($1.2)) }
    }
    
    func addItem(_ item: (String, Double, Int)) {
        if let index = cartItems.firstIndex(where: { $0.0 == item.0 }) {
            cartItems[index].2 += item.2
        } else {
...
struct CartView: View {
    ...
    var body: some View {
        VStack {
            ...
            List {
                ForEach(Array(cartManager.cartItems.enumerated()), id: \.element.0) { index, item in
                    HStack {
